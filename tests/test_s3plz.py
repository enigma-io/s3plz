...
class TestS3plz(unittest.TestCase):
	...
	def test_s3plz(self):
		...
		try:
			...
			obj1 = {"key": "value"}
			formatstring = 's3plztest-0.1.1/{@date_path}/{key}/{@uid}.json.gz'
			...
			for fp in plz.ls('s3plztest-0.1.1/'):
				print fp
				assert('value' in fp)

			# check streaming method / deserialization
			for fp, obj in plz.stream('s3plztest-0.1.1/'):
				assert("key" in obj)
			...
			obj1 = {"foo":"value"}
			fp = plz.put(obj1, "s3plztest-0.1.1/{foo}.json.gz", serializer="json.gz", **obj1)
			obj2 = plz.get(fp, serializer="json.gz")
			assert(obj1 == obj2)

			string1 = "hello world"
			fp = plz.put(string1, "s3plztest-0.1.1/string.zip", serializer="zip")
			...
			for fp in plz.ls('s3plztest-0.1.1/'):
				plz.delete(fp)
			assert(len(list(plz.ls('s3plztest-0.1.1/'))) == 0)
